...
def load_ev_spawn_scenarios(env) -> None:
    ...
    env.df_req_energy = env.df_req_energy.rename(
...
def generate_residential_inflexible_loads(env) -> np.ndarray:
    ...
    if desired_timescale > dataset_timescale:
        ...
        data = data.loc[
            data.index.repeat(dataset_timescale / desired_timescale)
    ...
    data["date"] = pd.date_range(
        start=dataset_starting_date,
        periods=data.shape[0],
        freq=f"{desired_timescale}min",
    )
    ...
    simulation_date = (
        f'{year}-{simulation_date.split("-")[1]}-{simulation_date.split("-")[2]}'
    )
    ...
    for i in range(number_of_transformers):
        new_data["tr_" + str(i)] = data.sample(
            10, axis=1, random_state=env.tr_seed
        ).sum(axis=1)
...
def generate_pv_generation(env) -> np.ndarray:
    ...
    data["electricity"] = (
        data["electricity"]
        .rolling(window=60 // desired_timescale, min_periods=1)
        .mean()
    )
    # use other type of smoothing
    data["electricity"] = (
        data["electricity"].ewm(span=60 // desired_timescale, adjust=True).mean()
    )
    ...
    for i in range(num_charging_stations):  # One EV per charging station
        ...
        for day in range(1, 8):
            ...
            for trip in sampled_row["trips"]:
                ...
                miles = float(trip[2])
                trips.append(
                    {"departure": departure, "arrival": arrival, "miles": miles}
                )
